    ...
    for r in range(L-1,0,-1):
        ...
        if r == L-1:
            for j in range(currNeurons):
                ...
                out = outputs[r][j]
                target = expected[j]
                a = -(expected[j]-outputs[r][j])
                # t = '%.32f' % a
                # print(t)
                b = outputs[r][j]*(1-outputs[r][j])
                # t = '%.32f' % b
                # print(t)
                # t = '%.32f' % (a*b)
                # print(t)
                # delta = -(expected[j]-outputs[r][j])*outputs[r][j]*(1-outputs[r][j])
                ...
                # t = '%.32f' % -(expected[j]-outputs[r][j])*outputs[r][j]*(1-outputs[r][j])
                # print(t)
                # t = '%.32f' % delta
                # print(t)
                deltas[r].append(a*b)
...
def calculateDelWeight(delWeights,deltas,outputs,layerInfo,L):
    ...
    return delWeights
def updateWeightsDirectly(weights, deltas, outputs, learning_rate, layerInfo, L):
    for r in range(L - 1, 0, -1):
        # print("r: " + repr(r) + 'th layer')
        currNeurons = layerInfo[r]
        prevNeurons = layerInfo[r - 1]
        for j in range(currNeurons):
            for k in range(prevNeurons):
                old = weights[r][j][k]
                change = deltas[r][j] * outputs[r - 1][k]
                weights[r][j][k] -= learning_rate* change
                t = '%.32f'%(change*learning_rate)
                tt = '%.32f'%old
                # print('∆w_' + repr(j) + repr(k) + ' ' + repr( t) + ' old ' + repr(tt)
...
L = 3
layerInfo = [2,2,2]
weights = [[],[[1.15,1.20],[1.25,1.3]],[[.4,.45],[.5,.55]]]
...
for i in range(10000):
    ...
    for  j in range(len(training_sets)):
        ...
        enablePrint()
        # print('\nupdate weights')
        updateWeightsDirectly(weights,deltas,outputs,learning_rate,layerInfo,L)
